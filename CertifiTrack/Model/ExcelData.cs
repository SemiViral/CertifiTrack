            ...
            using (FileStream file = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
                ...
                for (int row = 3; row <= worksheet.Dimension.End.Row; row++) {
                    if (worksheet.Cells[row,
                        1].Value == null) continue;

                    DeathCertificate cert = new DeathCertificate(Convert.ToDouble(worksheet.Cells[row, 1].Value),
                        worksheet.Cells[row, 2].Value, worksheet.Cells[row, 3].Value, worksheet.Cells[row, 4].Value,
                        worksheet.Cells[row, 5].Value, worksheet.Cells[row, 6].Value, worksheet.Cells[row, 7].Value,
                        worksheet.Cells[row, 8].Value, worksheet.Cells[row, 9].Value, worksheet.Cells[row, 10].Value,
                        worksheet.Cells[row, 11].Value, worksheet.Cells[row, 12].Value);

                    if (cert.Notes == null ||
                        (!cert.Notes.Contains("Done") && !cert.Notes.Contains("Out of State") &&
                         !cert.Notes.Contains("Rental") &&
